...
parser.add_argument(
    ...
    type=str,
    required=False,
    default='biodatlab/whisper-th-large-v3-combined',
    help='Huggingface model name to fine-tune. Eg: biodatlab/whisper-th-large-v3-combined'
)
parser.add_argument(
    '--language',
    type=str,
...
if len(args.train_datasets) != len(args.train_dataset_configs):
    raise ValueError(
        f"Ensure that the number of entries in the list of train_datasets equals the number of entries in the list of train_dataset_configs. Received {len(args.train_datasets)} entries for train_datasets and {len(args.train_dataset_configs)} for train_dataset_configs.")
if len(args.eval_datasets) != len(args.eval_dataset_configs):
    raise ValueError(
        f"Ensure that the number of entries in the list of eval_datasets equals the number of entries in the list of eval_dataset_configs. Received {len(args.eval_datasets)} entries for eval_datasets and {len(args.eval_dataset_configs)} for eval_dataset_configs.")

if len(args.train_datasets) != len(args.train_dataset_splits):
    raise ValueError(
        f"Ensure that the number of entries in the list of train_datasets equals the number of entries in the list of train_dataset_splits. Received {len(args.train_datasets)} entries for train_datasets and {len(args.train_dataset_splits)} for train_dataset_splits.")
if len(args.eval_datasets) != len(args.eval_dataset_splits):
    raise ValueError(
        f"Ensure that the number of entries in the list of eval_datasets equals the number of entries in the list of eval_dataset_splits. Received {len(args.eval_datasets)} entries for eval_datasets and {len(args.eval_dataset_splits)} for eval_dataset_splits.")

if len(args.train_datasets) != len(args.train_dataset_text_columns):
    raise ValueError(
        f"Ensure that the number of entries in the list of train_datasets equals the number of entries in the list of train_dataset_text_columns. Received {len(args.train_datasets)} entries for train_datasets and {len(args.train_dataset_text_columns)} for train_dataset_text_columns.")
...
tokenizer = WhisperTokenizer.from_pretrained(
    args.model_name, language=args.language, task="transcribe", cache_dir="/data/huggingface/hub")
...
def prepare_dataset(batch):
    ...
    audio = batch["audio"]
    print("prepare_dataset", audio["array"], audio["sampling_rate"])
